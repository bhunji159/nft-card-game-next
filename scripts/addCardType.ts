...
async function main() {
	...
	const uniqueCards = [
		{
			tokenId: 1,
			uri: "ipfs://QmZ5AnPhmFeezosBydKCN5ae21AGAQLoqmGhTrhKBvwk3F",
		},
		{
			tokenId: 2,
			uri: "ipfs://QmUCLwUWkvWkiVTggMSMMrmEuHaj7n8NnBcG7zmtFzAeEY",
		},
	];

	for (const card of uniqueCards) {
		const tx = await gameManager
			.connect(deployer)
			.addUniqueCard(card.tokenId, card.uri);
		await tx.wait();
		console.log(
			`Unique Card Registered: tokenId ${card.tokenId}, URI: ${card.uri}`
		);
	}
	...
	for (const uri of multiCardURIs) {
		...
		// const event = receipt.events?.find((e: any) => e.event === undefined); // fallback
